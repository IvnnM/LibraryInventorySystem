...
<div class="container-fluid" id="headerCont">
      ...
      <select class="form-select" id="sortSelect" aria-label="Sort by Transaction Type" style="margin-bottom: 10px;">
        ...
        <?php
          ...
          if ($result->num_rows > 0) {
              ...
              echo "<table class='table table-dark table-hover' id='inventoryTable'>";
              echo "<tr class='table-header'><th>Book ID</th><th>Title</th><th>Description</th><th>Stocks</th><th></th><th></th></tr>";
  ...
  $('#sortSelect').change(function() {
    ...
    if (selectedOption === "Out of Stock") {
      table.find('tr').show();
      table.find('tr:not(.table-header):not(:has(td:contains("Out of Stock")))').hide();
    } else {
      table.find('tr').show();
    }
  });
